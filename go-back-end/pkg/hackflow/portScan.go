...
func (p *portScanner) genTargets(HostCh chan interface{}, portRange string) (chan *IPAndPort, error) {
	//1.解析端口范围
	portList, err := parsePortsList(portRange)
	if err != nil {
		return nil, err
	}
...
func (p *portScanner) ConnectScan(config *ScanConfig) (IPAndPortCh, error) {
	//生产者：生产IP和端口对
	targetCh, err := p.genTargets(config.HostCh, config.PortRange)
	if err != nil {
		return nil, err
	}
	//消费者：消费IP和端口对
		...
		LOOP:
			for {
				...
				case ipAndPort, ok := <-targetCh:
					...
					p.doConnectScan(ipAndPort.IP, ipAndPort.Port)
				}
			}
		}()
	}
